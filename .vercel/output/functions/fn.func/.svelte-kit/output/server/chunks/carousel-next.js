...
import { o as cn, B as Button } from "./messages.js";
...
const formatUrl$3 = ({ host, cloudName, assetType, deliveryType, signature, transformations = {}, version: version2, id, format }) => {
  if (format) {
...
const generate$6 = ({ base, width, height, format, params }) => {
  ...
  return new URL(formatUrl$2(props17));
};
...
const transform$f = ({ url: originalUrl, width, height }) => {
  const url = toUrl(originalUrl);
  setParamIfDefined(url, "width", width, true, true);
  if (width && height) {
...
const generate$5 = ({ base, width = 0, height = 0, format, params = {} }) => {
...
const generate$4 = ({ base, width, params: { quality = 75, root = "_vercel" } = {} }) => {
...
const transform$c = ({ url, width, cdn }) => {
...
const transform$b = (params) => transform$c({ ...params, cdn: "nextjs" });
...
const generate$3 = ({ base, width, height, format, params }) => {
  const parsed = parse$3(base.toString());
  const props17 = {
    transformations: {},
    ...parsed.params,
    ...params
...
const transform$5 = ({ url: originalUrl, width, height, format = "auto" }) => {
...
const generate$2 = ({ base: id, width, height, format, params }) => {
...
const transform$4 = (options) => {
  ...
  if (isIpxUrl) {
    const parsed = parse$2(url.replace(base, ""));
    return generate$2({
      ...parsed,
      ...options,
      params: {
...
const generate$1 = ({ base, width, height, format, params: { site, quality, ...params } = {} }) => {
...
const transform$2 = (options) => {
  ...
  if (url.pathname.startsWith("/.netlify/images")) {
    const { params, base, format } = parse$1(url);
    return generate$1({
      base,
      format,
      ...options,
