...
<Carousel.Root class="mx-auto max-w-[700px] w-full rounded-sm bg-off-blue py-8 md:w-2/3 " {opts} bind:api>
	<Carousel.Content class="relative px-3 py-10 ">
		{#each reasons as { src, title, description }}
			<Carousel.Item class="flex flex-col items-center  justify-center md:flex-row">
				<img class="w-1/4" {src} alt="decorative" />
				<div class="flex px-10 flex-col items-center justify-center py-5">
					<div class="mx-auto ">
						<h4 class="pb-5 font-playfair text-2xl uppercase">{title}</h4>
	...
	<div class="absolute inset-x-0 bottom-5 mx-auto flex w-1/2 justify-center py-2">
